...
async function json(data: any, status = 200, headers: Record<string, string> = {}) {
...
export const onRequest: PagesFunction = async (ctx) => {
  ...
  const sbAdmin = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_KEY)
  ...
  if (url.pathname === '/api/settings/public' && request.method === 'GET') {
    const { data, error } = await sbAnon.from('SiteSetting').select('*').eq('key', 'default').single()
    if (error) return json({ error: error.message }, 500, corsHeaders)
    return json({ settings: data || {} }, 200, corsHeaders)
  }

  if (url.pathname === '/api/universities' && request.method === 'GET') {
    const { data, error } = await sbAnon.from('University').select('*').eq('active', true).order('created_at', { ascending: false })
    if (error) return json({ error: error.message }, 500, corsHeaders)
  ...
  if (url.pathname.startsWith('/api/universities/') && request.method === 'GET') {
    ...
    const { data: uni, error } = await sbAnon.from('University').select('*').eq('slug', slug).single()
    if (error || !uni || !uni.active) return json({ error: 'University not found' }, 404, corsHeaders)
    const { data: feeRows } = await sbAnon.from('Fee').select('*').eq('university_id', uni.id).order('year', { ascending: true })
  ...
  if (url.pathname === '/api/applications' && request.method === 'POST') {
    ...
    const { data, error } = await sbAdmin.from('Lead').insert([insert]).select().single()
  ...
  if (url.pathname === '/api/uploads/single' && request.method === 'POST') {
    ...
    const { data, error } = await sbAdmin.storage.from(bucket).upload(fileName, new Uint8Array(buf), { contentType: file.type, upsert: false })
    if (error) return json({ error: error.message }, 500, corsHeaders)
    const { data: pub } = sbAnon.storage.from(bucket).getPublicUrl(fileName)
